...
impl ProjectDeploymentContext {
    pub fn find_user_session(&self) -> Option<UserSession> {
        Some(UserSession {
...
impl ContainerDeployer {
    ...
    pub async fn call_llm_for_deployment(context: &ProjectDeploymentContext) -> Result<String, String> {
        ...
        let mut llm_client = if model_base_url.is_empty() || model_base_url.contains("127.0.0.1") {
            LlmProxy::for_local(None,None)
        } else {
            LlmProxy::for_openai(model_base_url.to_string(), model_api_key.to_string(), model_name.to_string())
        };
        

        // 3. 从容器 MCP-SSE 服务动态获取工具列表
        // 工具列表完全从容器获取，不使用写死的 fallback，因为工具会更新
        let tool_executor = ToolExecutor::new(HashMap::new(), &mcp_server_url.as_str());
        println!("[DEBUG] ToolExecutor created with URL: {}", mcp_server_url);
        let mut count = 0;
        //尝试3分钟
        let max_attempts = 60;
        loop {
                let result = tool_executor.list_tools(Some(mcp_server_url.clone())).await;
                match result {
                Ok(t) => {
        ...
        let user_session = context.find_user_session();
        let tools = match tool_executor.list_tools(Some(mcp_server_url.clone())).await {
            Ok(t) => {
                println!("[DEBUG] Retrieved {} tools from container MCP-SSE", t.len());
                t
            },
        ...
        // let workdir = Some(context.debug_dir.clone());
        let mut stream = llm_client
            .chat_stream(&tool_executor, &mut messages, Some(&tools))
            .await;
        ...
        while let Some(data) = stream.next().await {
            match data {
                Ok(data) => {
                    full_content.push_str(&data.content.clone());
                    if data.done {
                        break;
                    }
                }, 
