...
use shared::models::{
    ...
    FileAssignmentInfo, ContainerConfigInfo,StreamResponse
};
    ...
    if !unassigned_files.is_empty() {
        ...
        if !container_config_infos.is_empty() && !file_infos.is_empty() {
            if let Some(model_id) = project.model_id {
                ...
                let mcp_servers = repo.get_mcp_servers().await.map_err(|e| ServiceError::InvalidInput(e.to_string()))?;
                let servers = mcp_servers.iter().map(|x| (x.id,x.url.to_string())).collect();
                let mut llm_client = LlmClient::new(servers, &model.access_url, &model.api_key, &model.name);
...
pub async fn workspace_chat_stream(
    ...
    // let mut tools = match tool_executor.list_tools(Some(mcp_server_url.clone())).await {
    //     Ok(t) => {
    ...
    let mcp_servers = repo.get_mcp_servers().await;
    // 5. 创建 LLM 客户端
    let servers = match mcp_servers {
        Ok(server) => {
            server.iter().map(|x| (x.id,x.url.to_string())).collect()
        },
        Err(_) => {
            HashMap::new()
        }
    ...
    let stream = async_stream::stream! {
        ...
        let tool_executor = ToolExecutor::new(servers.clone(), &mcp_server_url);
        let mut tools = tool_executor.list_tools(Some(mcp_server_url.clone())).await;

        let mut llm_client = LlmClient::new(servers.clone(), &model.access_url.clone(), &model.api_key.clone(), &model.name.clone());
        
        // 6. 调用 LLM + 工具执行循环
        // 没有工具，使用普通聊天流
        let stream = llm_client.get_proxy().chat_stream(
                                        &tool_executor
                                        ,&mut messages
                                        , match tools {
                                            Err(_) => None,
                                            Ok(ref tools) => Some(tools)
                                        }).await ;
        pin_mut!(stream);
...
pub async fn assign_files_to_containers(
    ...
    let mut llm_client = LlmClient::new(servers,&model.access_url.clone().as_str(), &model.api_key.clone().as_str(), &model.name.clone().as_str());
